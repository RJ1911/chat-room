        ...
        if (!email || !password) {
            ...
            if (!user) {
                ...
                if (!validateUser) {
                    ...
                    const JWT_SECRET_KEY = process.env.JWT_SECRET_KEY || "THIS_A_JWT_SECRET_KEY";
...
app.post('/api/message/', async (req, res) => {
    try {
        ...
        if (!senderId || !message) res.status(400).send({ message: 'Please fill required fields' });
...
app.get('/api/message/:conversationId', async (req, res) => {
    try {
        ...
        if (conversationId === 'new') {
            const checkExistingConversation = await Conversations.find({ members: { $all: [req.query.senderId, req.query.receiverId] } })
            if (checkExistingConversation.length > 0) {
                checkMessages(checkExistingConversation[0]._id)
            }
            else {
